...
export const form2search = {
  ...
  numberString: (obj, field) => {
    ...
    const value = obj[field];
    if (testPropTypes(value, 'string') === false) { return; }

    const convertValue = Number(value);
    if (testPropTypes(convertValue, 'NaN')) { return; }

    obj[field] = convertValue;
  },
  sourceId: (obj, field, mapping) => {
    if (obj === undefined) { return; }

    const value = obj[field];
    if (value === undefined) { return; }

    const item = mapping[obj[field]];
    if (item === undefined) { return; }

    obj[field] = item.origin.sourceId;
  }
};
...
export const search2form = {
  ...
  numberString: (value) => {
    ...
    if (testPropTypes(value, 'number') === false) { return; }
...
export function localVerifyToken() {
  ...
  // 获取token字符串
  const authorizationString = localStorage.getItem(LOCALSTORAGEKEY_AUTHORIZATION);
...
/* 组织架构树 */
export function getTreeOrgFrame(needRoot) {
  const orgframeString = localStorage.getItem(SEC_ORG_FRAME);
  const orgframeArray = JSON.parse(orgframeString);

  let result = null;
  
  // 过滤根节点
  if (needRoot !== true) {
    result = orgframeArray.filter(item => item.id !== item.parentId);
  }
  
  return tree(treenodeify(result));
}
